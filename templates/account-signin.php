...
<?php if ( is_user_logged_in() ) : ?>
	...
	<fieldset class="fieldset-logged_in">
		...
		<div class="field account-sign-in">
			...
			<a class="button" href="<?php echo esc_url( apply_filters( 'submit_job_form_logout_url', wp_logout_url( get_permalink() ) ) ); ?>"><?php esc_html_e( 'Sign out', 'wp-event-manager' ); ?></a>
		</div>
	</fieldset>
...
<?php else :
	...
	<fieldset class="fieldset-login_required">
		...
		<div class="field account-sign-in">
			<a class="button" href="<?php echo esc_url( apply_filters( 'submit_job_form_login_url', wp_login_url( get_permalink() ) ) ); ?>"><?php esc_html_e( 'Sign in', 'wp-event-manager' ); ?></a>
			...
			<?php elseif ( $account_required ) : ?>
				...
				<?php echo wp_kses_post( apply_filters( 'submit_job_form_login_required_message',  __( 'You must sign in to create a new listing.', 'wp-event-manager' ) ) ); ?>
	...
	if ( ! empty( $registration_fields ) ) {
		foreach ( $registration_fields as $key => $field ) {
			...
			<fieldset class="fieldset-<?php echo esc_attr( $key ); ?>">
				<label
					for="<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $field[ 'label' ] ) . wp_kses_post( apply_filters( 'submit_job_form_required_label', $field[ 'required' ] ? '' : ' <small>' . __( '(optional)', 'wp-event-manager' ) . '</small>', $field ) ); ?></label>
