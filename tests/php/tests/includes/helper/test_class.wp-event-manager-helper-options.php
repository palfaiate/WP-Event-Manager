 ...
 */
class WP_Test_WP_event_Manager_Helper_Options extends WPJM_Helper_Base_Test {
	/**
	 * @since 1.29.0
	 * @covers WP_event_Manager_Helper_Options::update
	 */
	public function test_update_simple() {
		$this->setup_master_option();
		WP_event_Manager_Helper_Options::update( 'test', 'licence_key', 'new-value' );
	...
	/**
	 * @since 1.29.0
	 * @covers WP_event_Manager_Helper_Options::get
	 */
	public function test_get_return_default() {
		$result_expected = 'simple';
		$result          = WP_event_Manager_Helper_Options::get( 'test', 'licence_key', $result_expected );
	...
	public function test_get_return_value() {
		$this->setup_master_option();
		$result = WP_event_Manager_Helper_Options::get( 'test', 'licence_key', 'simple' );
	 ...
	 * @since 1.29.0
	 * @covers WP_event_Manager_Helper_Options::get
	 * @covers WP_event_Manager_Helper_Options::attempt_legacy_restore
	 */
	public function test_get_return_legacy() {
		$this->setup_legacy_options();
		$licence_key_result = WP_event_Manager_Helper_Options::get( 'legacy', 'licence_key', 'simple' );
		$this->assertEquals( 'legacy-abcd', $licence_key_result );

		$email_result = WP_event_Manager_Helper_Options::get( 'legacy', 'email', 'simple' );
		$this->assertEquals( 'legacy@test.dev', $email_result );

		$errors_result = WP_event_Manager_Helper_Options::get( 'legacy', 'errors', 'simple' );
		$this->assertEquals( 'legacy-errors', $errors_result );

		$errors_hide_key_notice = WP_event_Manager_Helper_Options::get( 'legacy', 'hide_key_notice', 'simple' );
		$this->assertEquals( 'legacy-hide', $errors_hide_key_notice );
	}

	/**
	 * @since 1.29.0
	 * @covers WP_event_Manager_Helper_Options::delete
	 */
	public function test_delete_simple() {
		$this->setup_master_option();
		$result = WP_event_Manager_Helper_Options::delete( 'test', 'licence_key' );
	...
	private function setup_master_option( $value = null ) {
		...
		update_option( 'event_manager_helper', $value );
	}

	private function get_master_option() {
		return get_option( 'event_manager_helper', [] );
	}
}
