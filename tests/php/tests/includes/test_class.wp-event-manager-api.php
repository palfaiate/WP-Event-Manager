...
class WP_Test_WP_event_Manager_API extends WPJM_BaseTest {
	/**
	 * Tests the WP_event_Manager_API::instance() always returns the same `WP_event_Manager_API` instance.
	 *
	 * @since 1.26.0
	 * @covers WP_event_Manager_API::instance
	 */
	public function test_wp_event_manager_api_instance() {
		$instance = WP_event_Manager_API::instance();
		// check the class.
		$this->assertInstanceOf( 'WP_event_Manager_API', $instance, 'event Manager API object is instance of WP_event_Manager_API class' );

		// check it always returns the same object.
		$this->assertSame( WP_event_Manager_API::instance(), $instance, 'WP_event_Manager_API::instance() must always return the same object' );
	}

	/**
	 * @since 1.26.0
	 * @covers WP_event_Manager_API::add_query_vars
	 */
	public function test_add_query_vars() {
		...
		$this->assertContains( 'event-manager-api', $new_vars );
		$this->assertContains( 'existing-var', $new_vars );
	}

	/**
	 * @since 1.26.0
	 * @covers WP_event_Manager_API::api_requests
	 */
	public function test_valid_api_requests() {
		...
		$wp->query_vars['event-manager-api'] = $handler_tag;
